...
def aggregation_function2(scores, num_segments, a):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--shuffle_frames', type=bool, default=False,
                        help='Whether to randomly shuffle clip frames during training to ignore temporal information.')
    ...
    parser.add_argument('--input-size', type=int, default=256,
                        help="Size of the model's input images (squares)")
    ...
    print_model_summary(model, args.clip_length, args.crop_size, print_model=False)
    model = model.to(args.device)

    # Init WandB
    WANDB_PROJECT = "c6-week6"
    WANDB_ENTITY = "luisgogu2001"
    run_name = f"small_3D_resnet"
    ...
    for epoch in range(args.epochs):
        ...
        mean_train_loss, train_acc = train(model, loaders['training'], optimizer, loss_fn, args.device, description=description)

        # Validation
        if epoch % args.validate_every == 0:
            description = f"Validation [Epoch: {epoch+1}/{args.epochs}]"
            mean_val_loss, val_acc = evaluate(model, loaders['testing'], loss_fn, args.num_segments, args.num_spatial_crops, args.device, description=description)
    ...
    mean_test_loss, test_acc = evaluate(model, loaders['testing'], loss_fn, args.num_segments, args.num_spatial_crops, args.device, description=f"Testing")
