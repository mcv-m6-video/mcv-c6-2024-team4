...
class HMDB51Dataset(Dataset):
    ...
    def __init__(
        ...
        self.transform_of = Raft_Large_Weights.DEFAULT.transforms()
        self.model_of = raft_large(weights=Raft_Large_Weights.DEFAULT, progress=False).to(self.device)
        self.model_of.eval()
    ...
    def get_of_batch(self, batched_clips):

        # Compute optical flow
        batch_of = torch.zeros((batched_clips.shape[0], 2, batched_clips.shape[2], batched_clips.shape[3], batched_clips.shape[4]), dtype=torch.float32, device=self.device)

        for i in range(batched_clips.shape[2]-1):
            with torch.no_grad():
                img1, img2 = self.transform_of(batched_clips[:,:,i], batched_clips[:,:,i+1])
                of = self.model_of(img1.to(self.device), img2.to(self.device))[-1]
                batch_of[:, :, i+1] = of
