...
def run(config=CONFIG_PATH, weight=None, source:str=0, display=True, save=True):
    ...
    if isinstance(source, str):
        if source.isnumeric():
            source = int(source)
    #source = '/home/tda/Desktop/Master/C6/mcv-c6-2024-team4/data/S03/c010/vdo.avi'
    ...
    if save:
        ...
        #os.makedirs("./result")
        video_name = f"./result/{name}_deepsort_pred.avi"
        record = cv.VideoWriter(video_name, 
    ...
    name = source[-16:-13] + '_' + source[-12:-8]
    #file_path = f"./result/{name}_deepsort_pred.txt"  # Define the file path
    file_path = f"./result/{name}_deepsort_pred.pkl"  # Define the file path
    f = open(file_path, 'wb')
    pred_results = {}
    ...
    while(vid.isOpened()):
        ...
        if ret is True:
           ...
           # print(predictions[0].boxes)
            ...
            # Results
            result = predictions[0].boxes.xywh.cpu().numpy()
            conf = predictions[0].boxes.conf.cpu().numpy()
            print(result, conf)
            #print("eee", result, conf)
            ...
            if len(track_result) > 0:
                ...
                for i in range(len(track_result)):
                    ...
                    #f.write(str(frame_count) + ',' + str(x1) + ',' + str(y1) + ',' + str(x2) + ',' + str(y2) + ',' + str(track_id[i])+'\n')
                    pred_results[frame_count].append([x1, y1, x2, y2, track_id[i]])
